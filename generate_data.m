...
%Sampled time
t=0:2:3600; %6 minutes
x=t/10000;

%Data trends
trend.temp_motor=sin(2*pi*5*x) + 2*sin(2*pi*12*x);
trend.temp_comp=sin(2*pi*5*x) + 2*sin(2*pi*12*x);
...
for sheet=1:1
    ...
    data=[t;temp.motor; temp.comp;rpm.motor;rpm.comp;tension;vibration]';
    save logg.mat data
...
xlabel('time');
ylabel('temp_{comp}');
ax.two=subplot(2,3,3);
plot(data(:,1),data(:,4));
xlabel('time');
ylabel('rpm_{motor}');
ax.three=subplot(2,3,4);
plot(data(:,1),data(:,5));
xlabel('time');
ylabel('rpm_{comp}');
ax.four=subplot(2,3,5);
plot(data(:,1),data(:,6));
xlabel('time');
ylabel('tension');
ax.five=subplot(2,3,6);
plot(data(:,1),data(:,7));
xlabel('time');
ylabel('vibration');
